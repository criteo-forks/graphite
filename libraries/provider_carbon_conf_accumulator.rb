...
class Chef
  class Provider
    class GraphiteCarbonConfAccumulator < Chef::Provider
      ...
      def action_create
        carbon_resources = %w(cache relay aggregator aggregator_cache).map { |r| "graphite_carbon_#{r}".to_sym }
        ...
        new_resource.updated_by_last_action(file_resource.updated_by_last_action?)
      end
    end
  end
end
